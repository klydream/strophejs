...
import http, sys, random, binascii
...
class BOSHClient:
    ...
    def request_bosh_session(self):
        ...
        xml_stanza = "<body rid='%s' xmlns='http://jabber.org/protocol/httpbind' to='%s' xml:lang='en' wait='60' hold='1' window='5' content='text/xml; charset=utf-8' ver='1.6' xmpp:version='1.0' xmlns:xmpp='urn:xmpp:xbosh'/>" % (
    ...
    def xmpp_disco(self):
        ...
        xml_stanza = self.wrap_stanza_body(
            "<iq type='get' from='%s' to='%s' id='info1'><query xmlns='http://jabber.org/protocol/disco#info'/></iq>" % (
    ...
    def xmpp_disco_node(self, node_name):
        ...
        xml_stanza = self.wrap_stanza_body(
            "<iq type='get' from='%s' to='%s' id='info2'><query xmlns='http://jabber.org/protocol/disco#info' node='http://jabber.org/protocol/%s'/></iq>" % (
    ...
    def authenticate_xmpp(self):
        ...
        if 'DIGEST-MD5' in self.server_auth:
            ...
            xml_stanza = self.wrap_stanza_body('',
                                               "to='%s' xml:lang='en' xmpp:restart='true' xmlns:xmpp='urn:xmpp:xbosh'" % self.jid.host)
            data = self.send_request(xml_stanza)
            ...
            xml_stanza = self.wrap_stanza_body(
                "<iq id='bind_1' type='set' xmlns='jabber:client'><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'><resource>%s</resource></bind></iq>" % self.resource)
            data = self.send_request(xml_stanza)
            ...
            auth_str = b"\000"
            auth_str += self.jid.user.encode('utf-8')
            auth_str += b"\000"
            try:
            ...
            ttt = binascii.b2a_base64(auth_str)
            xml_stanza = self.wrap_stanza_body(
                    "<auth xmlns='urn:ietf:params:xml:ns:xmpp-sasl' mechanism='PLAIN'>%s</auth>" % bytes.decode(ttt))

            response = self.send_request(xml_stanza)
            self.log('Reponse to PLAIN authentication: %s' % response)
            # Request authentication fields
            xml_stanza = self.wrap_stanza_body(
                "<iq type='get' to='%s' id='auth1'><query xmlns='jabber:iq:auth'/></iq>" % self.jid.host)
            data = self.send_request(xml_stanza)
...
class AdminBOSHClient(BOSHClient):
    ...
    def add_user(self, username, password):
        ...
        xml_stanza = self.wrap_stanza_body(
            "<iq from='%s' id='%s' to='%s' type='set' xml:lang='en'><command xmlns='http://jabber.org/protocol/commands' action='execute' node='http://jabber.org/protocol/admin#add-user'/></iq>" % (
    ...
    def get_registred_users(self):
        ...
        command.set_command(xmlns=NS_COMMANDS, action='execute',
